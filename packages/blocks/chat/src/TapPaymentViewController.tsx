...
import { ChargeResponse } from "../../tappaymentsintegration/src/types/types";
...
export default class TapPaymentViewController extends BlockComponent<
  Props,
  S,
  SS
> {
  ...
  async receive(from: string, message: Message) {
    ...
    if (
      ...
      message.getData(getName(MessageEnum.NavigationTapPaymentsViewMessageData))
    ) {
    ...
    if (message.id === getName(MessageEnum.RestAPIResponceMessage)) {
      ...
      if (apiCallId === this.getChargeApiCallId) {
        ...
        if (response.status === "CAPTURED") {
            let loyaltyPointText = response.meta.total_loyalty_points
            await setStorageData('loyaltyPoints',JSON.stringify(loyaltyPointText))        
  ...
  getCharge = () => {
    console.log(`${configJSON.retriveCharge}?charge_id=${this.state.chargeId}`, "====Charge API")
