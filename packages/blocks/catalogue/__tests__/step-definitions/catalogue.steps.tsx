...
import { catalogueListMock } from "../../__mock__/responses.mock";
...
defineFeature(feature, (test) => {
  ...
  test("User applied some filters", ({ given, when, then }) => {
    ...
    given("I am a user who applied filters", () => {
      screen = render(<Catalogue {...screenProps} />);
    });
  ...
  test("Guest user navigates to catalogue screen", ({ given, when, then }) => {
    let screen: ReturnType<typeof render>;
    const alertSpy = jest
    ...
    given("screen is loaded with data", () => {
      ...
      navMsg.initializeFromObject({
        [getName(MessageEnum.SubCategoryIdMessage)]: 10,
        [getName(MessageEnum.subCategoryNameMessage)]: "Dresses",
      });
      ...
      catalogueMsg.initializeFromObject({
        [getName(MessageEnum.RestAPIResponceDataMessage)]:
          catalogueMsg.messageId,
        [getName(MessageEnum.RestAPIResponceSuccessMessage)]: catalogueListMock,
      });
      screen.container.instance.getProductApiCallId = catalogueMsg.messageId;
      runEngine.sendMessage("UNIT TEST", catalogueMsg);
    });

    when("user clicks on wishlist button", () => {
      fireEvent.press(screen.getByTestId("wishlist-0"));
    });
  ...
  test("Authenticated user navigates to catalogue screen", ({
    given,
    when,
    then,
  }) => {
    ...
    when("user clicks on wishlist button", () => {
      screen.container.instance.state.stylistWishlist = {
        isStylist: true,
        wishlistId: 0,
      };
      fireEvent.press(screen.getByTestId("wishlist-0"));
      const wlMsg = new Message(getName(MessageEnum.RestAPIResponceMessage));
      wlMsg.initializeFromObject({
        [getName(MessageEnum.RestAPIResponceDataMessage)]: wlMsg.messageId,
        [getName(MessageEnum.RestAPIResponceSuccessMessage)]: catalogueListMock,
      });
      screen.container.instance.addWishlistApiCallId = wlMsg.messageId;
      runEngine.sendMessage("UNIT TEST", wlMsg);
    });

    then("catalogue is added to wishlist", () => {
      expect(screen.queryByTestId("filled")).toBe(null);
    });
    ...
    when("user clicks on wishlist button again", () => {
      ...
      screen.container.instance.removeWishlistApiCallId = wlMsg.messageId;
      runEngine.sendMessage("UNIT TEST", wlMsg);
    });

    then("catalogue is removed from wishlist", () => {
      expect(screen.queryAllByTestId("emptyHeart").length).toBe(0);
    });
    ...
    data: [
      {
        ...
        attributes: {
          ...
          is_wishlist: false,
          product_number: null,
          primary_image:"http://temp.image/rails/active_storage/blobs/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBb1lFIiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--6f7bfaa795d357a32a9ff749bbb13225a32ff296/profile.jpg",
          primary_discounted_percentage:"25.0",
          primary_main_price:"1000.0",
          primary_price:"750.0",
          gender: "female",
          brand_name: "AAHWAN",
          material: "Polyester ",
